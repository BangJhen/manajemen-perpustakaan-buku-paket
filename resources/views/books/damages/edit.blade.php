...
@section('title', 'Edit Laporan Kerusakan - ' . $book->title)
    ...
    <div class="col-lg-10">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-5">
                <form action="{{ route('books.damages.update', [$book, $damage]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="text-center mb-5">
                        ...
                        <h4 class="fw-bold text-dark">Edit Laporan Kerusakan</h4>
                        <p class="text-muted">Perbarui informasi detail kerusakan buku</p>
                    </div>
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select form-select-lg border-0 bg-light @error('damage_type') is-invalid @enderror" 
                                    ...
                                    <option value="Halaman Robek" {{ old('damage_type', $damage->damage_type) == 'Halaman Robek' ? 'selected' : '' }}>Halaman Robek</option>
                                    <option value="Halaman Hilang" {{ old('damage_type', $damage->damage_type) == 'Halaman Hilang' ? 'selected' : '' }}>Halaman Hilang</option>
                                    <option value="Cover Rusak" {{ old('damage_type', $damage->damage_type) == 'Cover Rusak' ? 'selected' : '' }}>Cover Rusak</option>
                                    <option value="Coretan/Vandalisme" {{ old('damage_type', $damage->damage_type) == 'Coretan/Vandalisme' ? 'selected' : '' }}>Coretan/Vandalisme</option>
                                    <option value="Basah/Terkena Air" {{ old('damage_type', $damage->damage_type) == 'Basah/Terkena Air' ? 'selected' : '' }}>Basah/Terkena Air</option>
                                    <option value="Penjilidan Lepas" {{ old('damage_type', $damage->damage_type) == 'Penjilidan Lepas' ? 'selected' : '' }}>Penjilidan Lepas</option>
                                    <option value="Lainnya" {{ old('damage_type', $damage->damage_type) == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                                </select>
                                @error('damage_type')
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select form-select-lg border-0 bg-light @error('severity') is-invalid @enderror" 
                                    ...
                                    <option value="ringan" {{ old('severity', $damage->severity) == 'ringan' ? 'selected' : '' }}>
                                        🟡 Ringan (Masih bisa digunakan)
                                    </option>
                                    <option value="sedang" {{ old('severity', $damage->severity) == 'sedang' ? 'selected' : '' }}>
                                        🟠 Sedang (Perlu perbaikan)
                                    </option>
                                    <option value="berat" {{ old('severity', $damage->severity) == 'berat' ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" 
                                       ...
                                       id="location" 
                                       name="location" 
                                       value="{{ old('location', $damage->location) }}"
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" 
                                       ...
                                       id="damage_date" 
                                       name="damage_date" 
                                       value="{{ old('damage_date', $damage->damage_date->format('Y-m-d')) }}"
                                       required>
                                @error('damage_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control border-0 bg-light @error('description') is-invalid @enderror" 
                                          ...
                                          rows="5" 
                                          required
                                          placeholder="Jelaskan secara detail kondisi kerusakan buku, bagian mana yang rusak, seberapa parah, dll.">{{ old('description', $damage->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" 
                                       ...
                                       id="reported_by" 
                                       name="reported_by" 
                                       value="{{ old('reported_by', $damage->reported_by) }}"
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select form-select-lg border-0 bg-light @error('status') is-invalid @enderror" 
                                    ...
                                    <option value="rusak" {{ old('status', $damage->status) == 'rusak' ? 'selected' : '' }}>Rusak (Belum Diperbaiki)</option>
                                    <option value="diperbaiki" {{ old('status', $damage->status) == 'diperbaiki' ? 'selected' : '' }}>Sudah Diperbaiki</option>
                                    <option value="tidak_dapat_diperbaiki" {{ old('status', $damage->status) == 'tidak_dapat_diperbaiki' ? 'selected' : '' }}>Tidak Dapat Diperbaiki</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" 
                                       ...
                                       id="repair_date" 
                                       name="repair_date" 
                                       value="{{ old('repair_date', $damage->repair_date?->format('Y-m-d')) }}">
                                @error('repair_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control border-0 bg-light @error('repair_notes') is-invalid @enderror" 
                                          ...
                                          name="repair_notes" 
                                          rows="4"
                                          placeholder="Jelaskan tindakan perbaikan yang dilakukan">{{ old('repair_notes', $damage->repair_notes) }}</textarea>
                                @error('repair_notes')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="d-grid gap-2 d-md-flex justify-content-md-end pt-4 border-top">
                        ...
                        <button type="submit" class="btn btn-lg btn-warning text-white px-5">
